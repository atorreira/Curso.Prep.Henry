...
function incrementarPorUno(array) {
  ...
  for (x=0;x!=array.length;x++){
    if ((array[x]-Math.floor(array[x])) ===0){
      array[x]=array[x]+1;
  }
  }
  return array;
...
function agregarNumeros(numeros) {
  ...
  for (let x=0;x!=numeros.length;x++){
    if (numeros[x]-Math.round(numeros[x])===0);{
      suma=suma+numeros[x];
    }
  }
  return suma;
...
function promedioResultadosTest(resultadosTest) {
  ...
  for (let x=0;x!=resultadosTest.length;x++){
    if (resultadosTest[x]-Math.round(resultadosTest[x])===0);{
...
function multiplicarArgumentos() {
  // Usa la palabra clave `arguments` para 
  //multiplicar todos los argumentos y devolver el producto
...
function continueStatement(numero) {
  ...
  //el valor 5, no se suma en ese caso y se continua con la siguiente iteración
